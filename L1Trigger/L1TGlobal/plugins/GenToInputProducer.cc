...
namespace l1t {
  ...
  private:
    ...
    int maxNumMuCands_;
    int maxNumMuShowerCands_; // added by RICK
    int maxNumJetCands_;
    int maxNumEGCands_;
    int maxNumTauCands_;
    ...
    // Tokens
    // edm::EDGetToken muShowerToken;  //addedy by RICK (from BXVectorInputProducer.cc but not used by GenToInputProducer.cc)
  ...
  GenToInputProducer::GenToInputProducer(const ParameterSet& iConfig) {
    //muShowerToken = consumes<BXVector<l1t::MuonShower>>(iConfig.getParameter<InputTag>("muShowerInputTag")); //added by RICK (from BXVectorInputProducer.cc but not used by GenToInputProducer.cc)
    ...
    maxNumMuShowerCands_ = iConfig.getParameter<int>("maxMuShowerCand"); // added by RICK (changed unsigned int to int)
  ...
  void GenToInputProducer::produce(Event& iEvent, const EventSetup& iSetup) {
    ...
    std::vector<l1t::MuonShower> muonShowerVec; //added by RICK
    ...
    std::unique_ptr<l1t::MuonShowerBxCollection> muonShowers(new l1t::MuonShowerBxCollection(0, bxFirst, bxLast)); // added by RICK
    ...
    for (int iMu = 0; iMu < numMuCands; iMu++) {
      ...
      bool mus0 = (iMu == 0) ? (bool) gRandom->Integer(2) : false;  // should be [0,1] = 1 bit; added by RICK
      bool mus1 = (iMu == 2) ? (bool) gRandom->Integer(2) : false;  // added by RICK
      bool mus2 = false; //gRandom->Integer(2);  // added by RICK
      bool musoot0 = (iMu == 4) ? (bool) gRandom->Integer(2) : false; // added by RICK
      bool musoot1 = (iMu == 6) ? (bool) gRandom->Integer(2) : false; // added by RICK
      bool musoot2 = false; //gRandom->Integer(2);  // added by RICK
      //Need to fill the MuonShower.h object with the above and pass that object to GtRecordDump

      cout << iMu << " = (MUS0,MUS1,MUSOOT0,MUSOOT1) = (" << mus0 << "," << mus1 << "," << musoot0 << "," << musoot1 << ") , qual = " << qual << endl; 
      ...
      l1t::MuonShower muShower(mus0,  // added by RICK
			      musoot0,
			      mus2,
			      musoot2,
			      mus1,
			      musoot1);
      muonShowerVec.push_back(muShower); // added by RICK
    }
    ...
    // Fill MuonShowers...added by RICK
    ...
    if (emptyBxTrailer_ <= (emptyBxEvt_ - eventCnt_)) {
      ...
      // this event is part of empty trailer...clear out data...added by RICK
      muonShowerVec.clear();
    }
    ...
    iEvent.put(std::move(muonShowers));  //added by RICK
    ...
    muonShowerVec_bxm2 = muonShowerVec_bxm1; //added by RICK
    ...
    muonShowerVec_bxm1 = muonShowerVec_bx0; //added by RICK
    ...
    muonShowerVec_bx0 = muonShowerVec_bxp1; //added by RICK
    ...
    muonShowerVec_bxp1 = muonShowerVec; //added by RICK
