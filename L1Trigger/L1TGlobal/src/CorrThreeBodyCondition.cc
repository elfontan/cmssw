...
const bool l1t::CorrThreeBodyCondition::evaluateCondition(const int bxEval) const {
  ...
  for (std::vector<SingleCombInCond>::const_iterator it0Comb = cond0Comb.begin(); it0Comb != cond0Comb.end();
	...
	std::cout << "Found all quantities for MU0" << std::endl;
    ...
    for (std::vector<SingleCombInCond>::const_iterator it1Comb = cond1Comb.begin(); it1Comb != cond1Comb.end(); 
	 it1Comb++) {
	  ...
	  std::cout << "Found all quantities for MU1" << std::endl;
	...
	if (cond2Categ == CondMuon) {
	  ...
	  std::cout << "Found all quantities for MU2" << std::endl;
	...
	std::cout << ">>>>>> Object involved in the three-body correlation condition are [" 
	...
	if (corrPar.corrCutType & 0x8) {
	  ...
	  std::cout << "    Testing the dimuon invariant mass between the FIRST PAIR 0-1 (" << lutObj0 << "," << lutObj1 << ") \n"
	  ...
	  std::cout << "####################################\n";
	  //LogDebug("L1TGlobal") << "    Testing Invariant Mass (" << lutObj0 << "," << lutObj2 << ") ["
	  std::cout << "    Testing the dimuon invariant mass between the SECOND PAIR 0-2 (" << lutObj0 << "," << lutObj2 << ") \n"
	  ...
	  std::cout << "####################################\n";
	  //LogDebug("L1TGlobal") << "    Testing Invariant Mass (" << lutObj1 << "," << lutObj2 << ") ["
	  std::cout << "    Testing the dimuon invariant mass between the THIRD PAIR 1-2 (" << lutObj1 << "," << lutObj2 << ") \n"
	  ...
	  if (preShift_01 == preShift_02  && preShift_01 == preShift_12 && preShift_02 ==preShift_12){
	    std::cout << "Check the preshift value: " << preShift_01 << " = " << preShift_02 <<" = " << preShift_12 << std::endl; 
